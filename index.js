...
const connect = async () => { 
        const filePath = path.join("./session", "creds.json");
        const fileData = await fs.promises.readFile(filePath, "utf8");
        console.log("Session Read Ayi ✅");
...
const handleConnectionUpdate = (conn) => async (s) => {
  ...
  switch (connection) {
    ...
    case "open":
      ...
      const cleanedJid = conn.user.id.includes(':') 
                        ? conn.user.id.split(':')[0] + '@s.whatsapp.net' 
                        : conn.user.id;
      const statusMessage = `*WhatsApp Bot Connected*\nVERSION: ${packageVersion}\nPLUGINS: ${totalPlugins}\nMODE: ${workType}`;
      await conn.sendMessage(cleanedJid, { text: statusMessage });
      break;
    case "close":
